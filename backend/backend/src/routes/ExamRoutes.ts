...
export default class ExamRoutes {
  ...
  private initializeRoutes() {
    ...
    this.router.get('/available/:courseId', async (req, res) => {
      try {
        const exams = await this.exam.getAvailableForStudent(parseInt(req.params.courseId));
        res.json(exams);
      } catch (error) {
    ...
    this.router.put('/:id', async (req, res) => {
      try {
        const updatedExam = await this.exam.update(
